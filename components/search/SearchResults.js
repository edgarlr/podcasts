...
const SearchResults = ({keyword}) => {
  ...
  const { data: channelsData, isLoading: channelsLoading } = useSearch(channelsUrl, 'channels', 4)

  const episodesUrl = `https://api.audioboom.com/audio_clips?find[query]=${keyword}&api_version=1`
  const { data: episodesData, isLoading: episodesLoading } = useSearch(episodesUrl, 'audio_clips', 4)

  if (!episodesLoading && !channelsLoading && episodesData.length === 0 && channelsData.length === 0) {
